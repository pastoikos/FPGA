...
typedef struct {
    u32 addr;
    u32 size;
} XBilateralfilterkernel_uio_map;
...
typedef struct {
    ...
    XBilateralfilterkernel_uio_map maps[ MAX_UIO_MAPS ];
} XBilateralfilterkernel_uio_info;

/***************** Variable Definitions **************************************/
static XBilateralfilterkernel_uio_info uio_info;
...
static int uio_info_read_name(XBilateralfilterkernel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XBilateralfilterkernel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XBilateralfilterkernel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XBilateralfilterkernel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XBilateralfilterkernel_Initialize(XBilateralfilterkernel *InstancePtr, const char* InstanceName) {
	XBilateralfilterkernel_uio_info *InfoPtr = &uio_info;
